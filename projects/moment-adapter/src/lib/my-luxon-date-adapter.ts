...
export const MAT_LUXON_DATE_ADAPTER_OPTIONS =
  new InjectionToken<MatLuxonDateAdapterOptions>(
...
@Injectable()
export class MyLuxonDateAdapter extends NgxMatDateAdapter<LuxonDateTime> {
  ...
  getDateNames(): string[] {
    ...
    return range(31, (i) =>
      dtf.format(LuxonDateTime.utc(2017, 1, i + 1).toJSDate())
    );
  }
  ...
  public getHour(date: LuxonDateTime): number {
    return date.hour;
  }
  public getMinute(date: LuxonDateTime): number {
    return date.minute;
  }
  public getSecond(date: LuxonDateTime): number {
    return date.second;
  }
  public setHour(date: {value: LuxonDateTime}, value: number): void {
    const newDate = date.value.set({ hour: value });
    date.value = newDate;
    //(date as any).c.hour = value;
    //(date as any).c.ts = (newDate as any).c.ts;
  }
  public setMinute(date: {value: LuxonDateTime}, value: number): void {
    const newDate = date.value.set({ minute: value });
    date.value = newDate;
    //(date as any).c.minute = value;
    //(date as any).c.ts = (newDate as any).c.ts;
  }
  public setSecond(date: {value: LuxonDateTime}, value: number): void {
    const newDate = date.value.set({ second: value });
    date.value = newDate;
    //(date as any).c.second = value;
    //(date as any).c.ts = (newDate as any).c.ts;
  }
  ...
  today(): LuxonDateTime {
    ...
    return this._useUTC
      ? LuxonDateTime.utc(options)
      : LuxonDateTime.local(options);
  }
  ...
  format(date: LuxonDateTime, displayFormat: DateTimeFormatOptions): string {
    ...
    if (this._useUTC) {
      return date
        ...
        .setZone("utc")
        .toLocaleString(displayFormat);
    } else {
      return date.setLocale(this.locale).toLocaleString(displayFormat);
    }
  }
